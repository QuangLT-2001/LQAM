...
import { getListManagerCourse, selectIsLoading, selectManagerCourse } from "features/auth/authSlice"
...
const ManagerCourseList = () => {
     ...
     useEffect(() => {
      ...
      let data = SelectManagerCourse.filter((item:any) => removeVietnameseTones(item.name).toUpperCase().includes(convertUpperCase));
      setDataFilter(data)
 }, [keyword, value])
 ...
 let count = data.length;
     ...
     return <ManagerCourseWrapper className="manager__course__wrapper">
          ...
          <div className="table__list w-100 p-3 bg-white" style={{overflowX: "auto", height: "calc(100vh - 200px)"}}>
               <table className="table w-100" style={{overflowX: "auto"}}>
                    <thead>
                         <tr>
                              {tableList.map(item => {
                                   return <th style={{width: item.width}} key={item.id}>{item.name}</th>
                              })}
                         </tr>
                    </thead>
                    <tbody>
                         {lstData.length ? lstData.map((item:any) => <ManagerCourseItem key={item.id} managerCourseItem={item}/>) : <tr>
