...
export let deleteContract = createAsyncThunk(
...
export let deleteLishMent = createAsyncThunk(
  'lishMent/deleteLishMent',
  async (params: any, thunkApi?: any) => {
    try {
      const respon = await axios.delete(`${BASE_URL}/${params.url}/${params.id}`);
      return respon;
...
export let getLishMentByCode = createAsyncThunk(
  'lishMent/getLishmentByCode',
...
export const getListManagerCourse = createAsyncThunk(
  'managerCourse/getListManagerCourse',
  (params: any) => {
    try {
...
export const postManagerCourse = createAsyncThunk(
  'managerCourse/postManagerCourse',
  (params: any) => {
    try {
...
export const getManagerCourseByCode = createAsyncThunk(
  'managerCourse/getManagerCourseByCode',
  (params: any) => {
    try {
...
export const deleteManagerCourse = createAsyncThunk(
  'managerCourse/deleteManagerCourse',
  (params: any) => {
    try {
...
export const putManagerCourse = createAsyncThunk(
  'managerCourse/putManagerCourse',
  (params: any) => {
    try {
...
export const postListImages = createAsyncThunk('imageSlide/postListImages', (params: any) => {
  try {
    const respon = axios
      .post(params.url, params.formData, params.options)
      .then((respon) => respon.data);
    return respon;
  } catch (err) {}
});
export const putManagerCourseAll = createAsyncThunk(
  'managerCourse/putManagerCourseAll',
  (params: any) => {
    try {
      const respon = axios.put(`${BASE_URL3}/${params.url}/${params.id}`, params.object);
      return respon;
...
export const postKpi = createAsyncThunk('Kpi/postKpi', (params: any) => {
  try {
    const respon = axios.post(`${BASE_URL3}/${params.url}`, params.obj);
    return respon;
  } catch (err) {}
});
export const getLstKpi = createAsyncThunk('Kpi/getLstKpi', (params: any) => {
  try {
    const respon = axios.get(`${BASE_URL3}/${params.url}`);
    return respon;
  } catch (err) {}
...
export const authSlice = createSlice({
  ...
  reducers: {
    ...
    deleteFile(state, action) {
      const data = state.imageSlide.filter(
        (item: any, index: number) => index !== action.payload.id
      );
    ...
    deleteFileSource(state, action) {
      state.lstFile = state.lstFile.filter(
        (item: any, index: any) => item.name !== action.payload.name
      );
    },
    ...
    deleteImageSlide(state, action) {
      state.lstFile = state.lstFile.filter(
        (item: any, index: any) => item.asset_id !== action.payload.id
      );
    },
  ...
  extraReducers(builder) {
    ...
    builder.addCase(postKpi.pending, (state, action) => {
      state.isLoading = true;
    });
    builder.addCase(postKpi.fulfilled, (state, action) => {
      state.isLoading = false;

      state.lstKpi.push(action.payload?.data);
    });

    builder.addCase(getLstKpi.pending, (state, action) => {
      state.isLoading = true;
    });
    builder.addCase(getLstKpi.fulfilled, (state, action) => {
      state.isLoading = false;

      state.lstKpi = action.payload?.data;
    });
